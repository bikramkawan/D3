...
const data = rawData.message.map(d=>({time: dateParse(d.time), count: d.wifiCount}))

const randomData = data.map(d=>({time: d.time, count: 0.8 * d.count}))

console.log(data, 'main')

const randomDate = d3.timeDay.range(new Date(2017, 1, 1), new Date(2017, 1, 30), 1);
const randBarData = randomDate.map(d=>({time: d, count: Math.random() * 100}))
